...
Wine orders on `/shop/wine` email **brett@tuckerfamilycharity.co.za** automatically when a customer submits the form. No extra setup is required for the default path (FormSubmit.co).
...
Submissions POST to FormSubmit.co, which emails the full order to **brett@tuckerfamilycharity.co.za**.
