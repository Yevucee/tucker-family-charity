...
/**
 * OPPORTUNITIES — "Looking for Work" form → Google Apps Script → Sheet tab "Job Seekers".
 * Set VITE_LOOKING_FOR_WORK_SUBMIT_URL (deployed web app /exec URL).
 * Placeholder until deployed: LOOKING_FOR_WORK_GOOGLE_SCRIPT_ENDPOINT_HERE
 *
 * POST body: application/x-www-form-urlencoded, field `json` (stringified):
 *   timestamp, fullName, email, phone, location, workType, skills, availability,
 *   cvLink (empty until CV upload), additionalNotes, status ("new"), internalNotes ("")
 * Sheet columns: Timestamp | Full Name | Email | Phone | Location | Type of Work Wanted |
 *   Skills / Experience | Availability | CV Link | Additional Notes | Status | Internal Notes
 *
 * Optional: VITE_LOOKING_FOR_WORK_SUBMIT_SECRET — include as `secret` in payload if script validates it.
 */
const RAW_LOOKING_FOR_WORK_SUBMIT_URL = String(
  import.meta.env.VITE_LOOKING_FOR_WORK_SUBMIT_URL ?? ""
).trim();

export const LOOKING_FOR_WORK_SUBMIT_URL = normalizeGasWebAppUrl(RAW_LOOKING_FOR_WORK_SUBMIT_URL);

export const LOOKING_FOR_WORK_SUBMIT_URL_REJECTED =
  RAW_LOOKING_FOR_WORK_SUBMIT_URL.length > 0 && LOOKING_FOR_WORK_SUBMIT_URL.length === 0;

export const LOOKING_FOR_WORK_SUBMIT_SECRET =
  import.meta.env.VITE_LOOKING_FOR_WORK_SUBMIT_SECRET ?? "";

/**
 * OPPORTUNITIES — "Offering Work" form → Google Apps Script → Sheet tab "Opportunities".
 * Set VITE_OFFERING_WORK_SUBMIT_URL (deployed web app /exec URL).
 * Placeholder until deployed: OFFERING_WORK_GOOGLE_SCRIPT_ENDPOINT_HERE
 *
 * POST body: application/x-www-form-urlencoded, field `json` (stringified):
 *   timestamp, contactName, organisation, email, phone, opportunityType, location, compensation,
 *   description, skillsRequired, preferredStartDate, additionalNotes, status ("new"), internalNotes ("")
 * Sheet columns: Timestamp | Contact Name | Organisation | Email | Phone | Type of Opportunity |
 *   Location | Paid / Unpaid / Volunteer / Training | Opportunity Description | Skills Required |
 *   Preferred Start Date | Additional Notes | Status | Internal Notes
 *
 * Optional: VITE_OFFERING_WORK_SUBMIT_SECRET
 */
const RAW_OFFERING_WORK_SUBMIT_URL = String(
  import.meta.env.VITE_OFFERING_WORK_SUBMIT_URL ?? ""
).trim();

export const OFFERING_WORK_SUBMIT_URL = normalizeGasWebAppUrl(RAW_OFFERING_WORK_SUBMIT_URL);

export const OFFERING_WORK_SUBMIT_URL_REJECTED =
  RAW_OFFERING_WORK_SUBMIT_URL.length > 0 && OFFERING_WORK_SUBMIT_URL.length === 0;

export const OFFERING_WORK_SUBMIT_SECRET = import.meta.env.VITE_OFFERING_WORK_SUBMIT_SECRET ?? "";
